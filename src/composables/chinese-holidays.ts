...
export function isChineseHoliday(day: Date): boolean {
  const holiday = findChineseDay(day)
  return holiday?.type === 'holiday'
}

export function isChineseWorkingDay(day: Date): boolean {
  const holiday = findChineseDay(day)
  return holiday?.type === 'workingday'
}

export function findChineseDay(day: Date): Holiday | undefined {
